...
class MathMethods {
 ...
 /*
     4. This question is very similar to the prior question (#3), except for one different. We will be working with Doubles as the type of our arguments, and not Ints. Create a function, isEqual(a:b:) which takes in two arguments. The parameter names should be a and b, both of type Double. This function will return back a Bool and determine whether or not the values passed in are equal to one another.
     */
     ...
     */
    func questionFive() -> Bool {
        
        let aDouble = 6.1
        let anInteger = 7
        
        if aDouble == Double(anInteger) {
            return true
        }
        else {
            return false
